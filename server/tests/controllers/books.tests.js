...
describe('Books Controller', () => {
  describe('Authenticated actions', () => {
    ...
    describe('POST /api/v1/books', () => {
      ...
      it(
        'should throw 403 error if an authenticated user tries to add a book',
        (done) => {
          request(app)
            .post('/api/v1/books')
      ...
      it('should throw 400 error if some fields are missing', (done) => {
        request(app)
          .post('/api/v1/books')
          .set('Authorization', `Token ${adminToken}`)
          .send(testData.missingQuantityBook)
          .end((err, res) => {
            expect(res.body.message).to.deep.equal('Unsuccessful');
            expect(res.status).to.equal(400);
            expect(res.body).to.have.property('error');
            expect(res.body.error).to.have.string('["quantity" is required]');
            done();
          });
      });
      it('should throw 409 error if book title already exists', (done) => {
        request(app)
          .post('/api/v1/books')
          .set('Authorization', `Token ${adminToken}`)
          .send(testData.validBook)
          .end((err, res) => {
            ...
            expect(res.body.error)
              .to.deep.equal('A book with this title already exists. ' +
                'Input a different title');
            done();
          });
      });
      it('should throw 400 error if imageURL is not valid', (done) => {
        request(app)
          .post('/api/v1/books')
          .send(testData.invalidImageURLBook)
          .set('Authorization', `Token ${adminToken}`)
          .end((err, res) => {
            expect(res.status).to.equal(400);
            expect(res.body.message).to.deep.equal('Unsuccessful');
            ...
            expect(res.body.error).to.have
              .string('["imageURL" must be a valid uri with a scheme '
              + 'matching the http|https pattern]');
            done();
          });
      });
      it(
        'should not allow a book to be added if no token is provided',
        (done) => {
          request(app)
            .post('/api/v1/books')
            .send(testData.validBook)
    ...
    describe('PUT /api/v1/books/:bookId', () => {
      it(
        'should update a book when an authenticated admin modifies a book',
        (done) => {
          request(app)
            .put('/api/v1/books/1')
            .set('Authorization', `Token ${adminToken}`)
            ...
            .end((err, res) => {
              ...
              expect(res.body.updatedBook).to.have.any.keys(
                'id',
                'title',
                'author',
                'subject',
      ...
      it(
        'should throw 400 error if admin updates a book with a negative bookId',
        (done) => {
          request(app)
            .put('/api/v1/books/-1')
            .set('Authorization', `Token ${adminToken}`)
      ...
      it(
        'should throw 404 error if admin wants to modify a non-existent book',
        (done) => {
          request(app)
            .put('/api/v1/books/54')
            .set('Authorization', `Token ${adminToken}`)
      ...
      it(
        'should throw 403 error if an authenticated user tries to update book ',
        (done) => {
          request(app)
            .put('/api/v1/books/1')
            .set('Authorization', `Token ${userToken}`)
      ...
      it(
        'should throw 400 error if any request field has an invalid data type',
        (done) => {
          request(app)
            .put('/api/v1/books/1')
            .set('Authorization', `Token ${adminToken}`)
    ...
    describe('POST  /api/v1/books/:bookId/review', () => {
      it(
        'should let an authenticated user review a book successfully',
        (done) => {
          request(app)
            ...
            .end((err, res) => {
              ...
              expect(res.body.reviewedBook.bookReviews[0].review).to
                .equal('An amazing read. I loved it!!!');
              done();
            });
        },
      );
      ...
      it(
        'should throw 400 error if bookId is not a positive integer',
        (done) => {
          request(app)
            .post('/api/v1/books/-1/review')
            .send({ review: 'An amazing read. I loved it!!!' })
      ...
      it('should return 400 error if review is empty', (done) => {
        request(app)
          ...
          .end((err, res) => {
            ...
            expect(res.body).to.have.property('error');
            expect(res.body.error).to.have
              .string('["review" is not allowed to be empty]');
            done();
          });
      });
        ...
        request(app)
          ...
          .end((err, res) => {
            ...
            expect(res.body.error).to
              .equal('Your review has already been created');
            done();
          });
      });
      it(
        'should return 404 error if a user tries to review a non-existent book',
        (done) => {
          request(app)
    ...
    describe('POST /api/v1/books/1/favorite', () => {
      ...
      it(
        `should return 201 status and favorite object
        ...
        (done) => {
          request(app)
            ...
            .end((err, res) => {
              ...
              expect(res.body.favorite).to.have.any.keys(
                'id',
                'bookId',
                'userId',
              );
      ...
      it(
        'should return 404 error if user tries to favorite a non-existent book',
        (done) => {
          request(app)
      ...
      it(
        'should return 409 error if book is already in user\'s favorites',
        (done) => {
          request(app)
            .post('/api/v1/books/1/favorite')
            .set('Authorization', `Token ${userToken}`)
    ...
    describe('POST /api/v1/books/:bookId/downvote', () => {
      ...
      it(
        'should return 201 status when an authenticated user upvotes a book',
        (done) => {
          request(app)
            .post('/api/v1/books/1/downvote')
            .set('Authorization', `Token ${userToken}`)
      ...
      it('should throw 409 error if user has downvoted book before', (done) => {
        request(app)
          ...
          .end((err, res) => {
            ...
            expect(res.body.message).to.deep.equal('Unsuccessful');
            expect(res.body.error).to.deep
              .equal('You have already downvoted this book');
            done();
          });
      });
      it(
        `should throw 404 error if a user tries
        to downvote a non-existent book`,
        (done) => {
          request(app)
      ...
      it('should throw 409 error if user has upvoted book before', (done) => {
        request(app)
          ...
          .end((err, res) => {
            ...
            expect(res.body.message).to.deep.equal('Unsuccessful');
            expect(res.body.error).to.deep
              .equal('You have already upvoted this book');
            done();
          });
      });
      it(
        'should throw 404 error if a user tries to upvote a non-existent book',
        (done) => {
          request(app)
    ...
    describe('DELETE /api/v1/books/:bookId', () => {
      ...
      it(
        `should throw 403 error if an authenticated
      ...
      it(
        'should let an authenticated admin delete a book in the database',
        (done) => {
          request(app)
            .delete('/api/v1/books/88')
            .set('Authorization', `Token ${adminToken}`)
      ...
      it(
        `should throw 404 error if an admin tries
        to delete a non-existent book`,
        (done) => {
          request(app)
            .delete('/api/v1/books/88')
            .set('Authorization', `Token ${adminToken}`)
      ...
      it(
        `should throw 400 error if an admin tries to delete a
        book with a negative bookId`,
        (done) => {
          request(app)
            ...
            .end((err, res) => {
              ...
              expect(res.body.message).to.equal('Unsuccessful');
              expect(res.body.error).to
                .equal('bookId must be a positive integer');
              done();
            });
        },
      );
      it(
        `should throw 400 error if an authenticated admin tries
        to delete a book with bookId that is not an integer`,
        (done) => {
          request(app)
  ...
  describe('Unauthenticated actions', () => {
    describe('GET /api/v1/books', () => {
      ...
      it(
        `should get all books with upvotes sorted in ascending order
         when the query is passed`,
        (done) => {
          request(app)
      ...
      it(
        'should get all books in a page when the page query is passed',
        (done) => {
          request(app)
      ...
      it(
        'should retrieve books as specified in the limit query',
        (done) => {
          request(app)
      ...
      it('should successfully return a book including its reviews', (done) => {
        request(app)
          ...
          .end((err, res) => {
            ...
            expect(res.body.book).to.have.any.keys(
              'bookId',
              'userId',
              'quantity',
              'author',
            ...
            expect(res.body.book.bookReviews[0]).to.have.any.keys(
              'bookId',
              'userId',
              'review',
              'createdAt',
      ...
      it('should throw 400 error if bookId is not an integer', (done) => {
        request(app)
          ...
          .end((err, res) => {
            ...
            expect(res.body).to.have.property('error');
            expect(res.body.error).to.have
              .string('["bookId" must be a number]');
            done();
          });
      });
      it(
        'should throw 404 error if book is not found in the database',
        (done) => {
          request(app)
    ...
    describe('POST /api/v1/books/search', () => {
      it(
        `should return books with a particular author
         when the author query is passed`,
        (done) => {
          request(app)
      ...
      it(
        `should return a book with a particular title
         when the title query is passed`,
        (done) => {
          request(app)
      ...
      it(
        `should return books with a particular subject
         when the subject query is passed`,
        (done) => {
          request(app)
