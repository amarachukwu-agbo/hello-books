...
describe('Users Controller', () => {
  ...
  describe('/api/v1/users/signup', () => {
    ...
    it('should throw 400 error if some fields are missing in the request', (done) => {
      request(app)
        .post('/api/v1/users/signup')
        .send(testData.incompleteUser)
        .end((err, res) => {
    ...
    it('should throw 400 error if a field contains an invalid data type', (done) => {
      request(app)
        .post('/api/v1/users/signup')
    ...
    it('should throw 409 error if user tries to sign up with an existing email', (done) => {
      request(app)
        .post('/api/v1/users/signup')
        .send(testData.secondUser)
        .end((err, res) => {
          ...
          expect(res.body).to.have.property('error');
          expect(res.body.message).to.equal('Unsuccessful');
          expect(res.body.error).to.equal('Email already exists. Input a different email');
          done();
        });
    });
  });
  ...
  describe('POST /api/v1/users/login', () => {
    ...
    it('should throw 401 error if user logins with a wrong password', (done) => {
      request(app)
        .post('/api/v1/users/login')
        .send({
          email: testData.secondUser.email,
        ...
        .end((err, res) => {
          ...
          expect(res.body.message).to.deep.equal('Unsuccessful');
          expect(res.body.error).to.deep.equal('Password provided does not match the user');
          expect(res.body).to.not.have.property('token');
          done();
        });
    });
  ...
  describe('GET /api/v1/:userId/favbooks', () => {
    ...
    it('should return 403 error if userId in params is not matched to user\'s token`s id', (done) => {
      request(app)
        .get('/api/v1/users/-2/favbooks')
        .set('Authorization', `Token ${userToken}`)
        .end((err, res) => {
  ...
  describe('POST /api/v1/users/:userId/borrow/:bookId', () => {
    ...
    it('should throw 403 error if the quantity of the book to be borrowed is 0', (done) => {
      request(app)
    ...
    it('should throw 404 error if the book to be borrowed does not exist', (done) => {
      request(app)
    ...
    it('should throw 404 error if some of the fields in the request are missing', (done) => {
      request(app)
        .post('/api/v1/users/2/borrow/930')
        .set('Authorization', `Token ${userToken}`)
        .send({
  ...
  describe('PUT /api/v1/users/:userId/borrow/:bookId', () => {
    ...
    it('should throw 403 if a user tries to handle a borrow request', (done) => {
      request(app)
        .put('/api/v1/users/2/borrow/94')
        .set('Authorization', `Token ${userToken}`)
        .send({
    ...
    it('should throw 400 error if the userId or bookId are not positive integers', (done) => {
      request(app)
    ...
    it('should throw 400 error if admin tries to handle a request with the wrong status', (done) => {
      request(app)
        .put('/api/v1/users/2/borrow/89')
        .set('Authorization', `Token ${adminToken}`)
        .send({
    ...
    it('should return 200 status if an admin accepts a borrow request', (done) => {
      request(app)
        .put('/api/v1/users/2/borrow/94')
        .set('Authorization', `Token ${adminToken}`)
        .send({
    ...
    it('should return 200 status if an admin declines a borrow request', (done) => {
      request(app)
        .put('/api/v1/users/2/borrow/89')
        .set('Authorization', `Token ${adminToken}`)
        .send({
    ...
    it('should throw 404 error if admin tries to handle a request already declined or accepted', (done) => {
      request(app)
        .put('/api/v1/users/2/borrow/89')
        .set('Authorization', `Token ${adminToken}`)
        .send({
  ...
  describe('POST /api/v1/users/:userId/return/94', () => {
    ...
    it('should throw 403 error if a user tries to return a book he has not borrowed', (done) => {
      request(app)
        ...
        .end((err, res) => {
          ...
          expect(res.body.message).to.equal('Unsuccessful');
          expect(res.body).to.have.not.have.property('returnRequest');
          expect(res.body.error).to.equal('You have not borrowed this book');
          done();
        });
    });
    it('should throw 400 error if a user adds an empty (optional)comment in the request', (done) => {
      request(app)
        .post('/api/v1/users/2/return/94')
        .set('Authorization', `Token ${userToken}`)
        .send({
    ...
    it('should return 201 status if a user sends a request to return a borrowed book', (done) => {
      request(app)
        .post('/api/v1/users/2/return/94')
        .set('Authorization', `Token ${userToken}`)
        .end((err, res) => {
    ...
    it('should return 400 error if bookId in params is not a positive integer', (done) => {
      request(app)
        .post('/api/v1/users/2/return/-94')
        .set('Authorization', `Token ${userToken}`)
        .end((err, res) => {
  ...
  describe('PUT /api/v1/users/:userId/return/:bookId', () => {
    ...
    it('should throw 403 if a user tries to handle a return request', (done) => {
      request(app)
        .put('/api/v1/users/2/return/94')
        .set('Authorization', `Token ${userToken}`)
        .send({
    ...
    it('should return 200 status if an admin accepts a return request', (done) => {
      request(app)
        .put('/api/v1/users/2/return/94')
        .set('Authorization', `Token ${adminToken}`)
        .send({
  ...
  describe('GET /api/v1/borrowRequests', () => {
    ...
    it('should return 200 status and requests if an admin tries to get borrow requests', (done) => {
      request(app)
        .get('/api/v1/borrowRequests')
        .set('Authorization', `Token ${adminToken}`)
        .end((err, res) => {
  ...
  describe('GET /api/v1/users/:userId', () => {
    ...
    it('should throw 403 error if userId in params is not matched to user\'s token\'s id', (done) => {
      request(app)
        .get('/api/v1/users/2')
        .set('Authorization', `Token ${adminToken}`)
        .end((err, res) => {
    ...
    it('should return user details of a user with an authenticated token', () => {
      request(app)
        .get('/api/v1/users/2')
        .set('Authorization', `Token ${userToken}`)
        .end((err, res) => {
          ...
          expect(res.body.user.userId).to.equal(2);
          ...
          expect(res.body.user).to.have.property('returnRequests');
          done();
        });
    });
  });
